...
var utils = (function() {
    //listToAry 类数组转换成数组
    //try满足不走catch  不满足就走catch  必须写e 即参数    如果try有错  catch就捕获e 并执行catch中的代码
    ...
    //getCss(ele,attr) 内嵌式或外链式里的css模式
    function getCss(ele, attr) {
        var res = null;
        if (window.getComputedStyle) {
            res = window.getComputedStyle(ele,null)[attr];
            //ele=属性 null=伪类 [attr]=属性
        } else {//ie6~8
            if (attr == "opacity") {
        ...
        //字符串理包含是数值|数值+单位 提取出数值部分  "100px"->100 "100"->100  "red"(原值返回)
    ...
    function  setCss(ele, attr, value) {
        //1.opacity 各个浏览器的区别设置
        //2.width|height|margin......若没加单位，需加个单位
        if (attr == "opacity") {
    ...
    //win(attr,value) 获取文档相关的值 例如一屏的宽高，整个文档的宽高，卷出去的宽高等
    ...
    function  getByClass(strClass,context) {
        ...
        for (var i = 0; i < allClassAry.length; i++){
            ...
            eles = ary;//把上次结果保存在else里，这样下次就能从上次结果中查找具有另一个类名的元素
        }
        return ary;
    }
...
console.log(utils.offset(father));
console.log(utils.offset(son));
